  ...
  const handleOrderClick = async (email, artisanId, artisan) => {
    ...
    if (token) {
      try {
        ...
        if (response.status === 201) {
          alert('Service added to your cart!');
          //navigate('/service-details', { state: { artisan, service } });
        ...
        {artisans.map((artisan) => {
          ...
          return (
            ...
            >
              ...
              <button
                onClick={() => handleOrderClick(artisan.user?.email, artisan.id,artisan)} // Pass artisan ID
                className="mt-auto bg-green-500 text-white px-4 py-2 rounded-lg transition-all duration-300 transform hover:bg-green-600"
                disabled={disabled}
              >
                {text}
...
const Artisans = () => {
  ...
  // Check if artisan is already in the cart (this can be done by just checking the cartStatus)
  const checkIfArtisanInCart = async (email) => {
  ...
  const handleOrderClick = async (email, artisanId, artisan) => {
    ...
    try {
      const response = await api.post('/employer/add_to_cart/', { artisan_email: email });
